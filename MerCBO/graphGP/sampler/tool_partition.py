...
def compute_unit_in_group(sorted_partition, n_vertices):
    ...
    :param sorted_partition: list of subsets, elements in a subset are ordered,
                              subsets are ordered by their smallest elements
    :param n_vertices:       1D list or array of per-variable cardinalities
    :return:                 list of unit-in-group lists (same ordering as sorted_partition)
    """
    import numpy as np

    # ensure we can index with a list of positions
    n_vertices = np.array(n_vertices, dtype=int)

    unit_in_group = []
    for subset in sorted_partition:
        # take the cardinalities of all but the first in reverse order,
        # cumulative-product them, flip back, and append a trailing 1
        sizes = n_vertices[subset]         # e.g. [3,4,5]
        tail = sizes[1:][::-1]             # e.g. [5,4]
        cum = np.cumprod(tail)             # e.g. [5,20]
        ind_units = list(np.flip(cum)) + [1]  # e.g. [20,5,1]
