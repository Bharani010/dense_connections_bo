...
def next_evaluation_with_thompson_sampling(input_data, output_data, inference_samples, partition_samples, log_noise_var, n_vertices, log_beta):
    ...
    beta =  np.array(np.exp(log_beta.numpy())) 
    beta = np.concatenate([beta,(np.outer(beta, beta))[ np.triu_indices(len(n_vertices), 1) ]]) #added this fixx
    ...
    upper_indices = np.triu_indices(len(n_vertices), 1)  # Fix 1: Use len(n_vertices)
    n = len(n_vertices)  # Define once for clarity and efficiency
    A = np.zeros((n, n))  # Fix 2: Use (n, n) for square matrix
    A[upper_indices] = theta[n:] * ((np.outer(beta, beta))[upper_indices])
    A.T[upper_indices] = theta[n:] * ((np.outer(beta, beta))[upper_indices])
    sdp_objective = theta.copy()
    sdp_objective[:n] *= np.sqrt(np.exp(-2*beta[np.argwhere(one_set)[:, 1]]))
    sdp_objective[:n] += np.sum(A, axis=0)
    sdp_objective[:n] = -2*sdp_objective[:n]
    double_idxs = np.argwhere(double_set)
    sdp_objective[n:] *= (4*np.sqrt(np.exp(-2*(beta[double_idxs[range(0, 2*len(double_set), 2), 1]] + beta[double_idxs[range(1, 2*len(double_set), 2), 1]]))))
    start_time = time.time()
    graphobj = graph_cuts.GraphCuts(n, sdp_objective)  # Fix 3: Use n
    ...
    x_new = torch.from_numpy(x_gc.astype(np.int64))
    print("Submodular Relaxation based AFO solution")
    print(x_new)

    if (torch.all(x_new == input_data, dim = 1).any()): #--- new logic to pick a random integer for each variable.
        print("Warning: AFO solution is a duplicate. Generating a random point.")
        # Create a new random vector respecting the cardinality of each variable.
        # n_vertices is a list like [2, 2, ..., 4, 4]
        random_vector = [np.random.randint(low=0, high=cardinality) for cardinality in n_vertices]
        x_new = torch.tensor(random_vector, dtype=torch.long)
        print("Returning random:", x_new)
        print("----------------------------------------------------")
